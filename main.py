...
def db_init():
    ...
    con.execute("CREATE TABLE customer AS SELECT * FROM 'dataset/cleaned_dataset/customers.parquet'")
    con.execute("CREATE TABLE orders AS SELECT * FROM 'dataset/cleaned_dataset/orders.parquet'")
    con.execute("CREATE TABLE products AS SELECT * FROM 'dataset/cleaned_dataset/products.parquet'")
...
selected_dashboard = option_menu(
    ...
    styles={
        ...
        "nav-link": {"font-size": "16px", "text-align": "center", "margin": "0px", "padding": "10px 0px","color": "black"},
        "nav-link-selected": {"background-color": "#FFDD00", "color": "white", "font-weight": "600"},
    }
)
...
category_list = cursor.execute('select distinct Category from flat').df()['Category'].tolist()
